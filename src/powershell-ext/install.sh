...
if [ ${#POWERSHELL_MODULES[@]} -gt 0 ]; then
    ...
    for i in "${modules[@]}"
    do
        module_attributes=(`echo ${i} | tr ':' ' '`)
        if [ ${#module_attributes[@]} -gt 1 ]; then
            echo "Installing ${module_attributes[0]} with version: ${module_attributes[1]}"
            pwsh -Command "Install-Module -Name ${module_attributes[0]} -RequiredVersion ${module_attributes[1]} -AllowClobber -Force -Scope AllUsers" || continue
        else
            echo "Installing ${module_attributes[0]} with version: latest"
            pwsh -Command "Install-Module -Name ${module_attributes[0]} -AllowClobber -Force -Scope AllUsers" || continue
        fi
    done
fi
